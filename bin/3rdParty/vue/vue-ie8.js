...
(function (global, factory) {
  ...
  (function()
  {
    ...
    (s = ua.match(/msie ([\d.]+)/)) ? ie = s[1] : null ;
    if(ie > 8)
    {
      return ;
    }

    var elementProto = (window.Node || window.Element).prototype;
    var textProto = Text.prototype;
    var defineElementProperty = function(name, defVal)
    {
      elementProto[name] = defVal;
      textProto[name] = defVal;
    }

    defineElementProperty("__v_anchor", false);
    defineElementProperty("_vue_directives", null);
    defineElementProperty("__v_trans", null);
    defineElementProperty("__v_pre__", false);
    defineElementProperty("__v_pre_root_now__", false);
    defineElementProperty("__vue__", null);
    defineElementProperty("__v_frag", null);
    defineElementProperty("_context", null);
    defineElementProperty("__v_model", null);
    // defineElementProperty("_value", null);
    // defineElementProperty("_trueValue", false);
    // defineElementProperty("_falseValue", false);
  })();

  function get(obj, key)
  {
    var val = obj[key];
    if(typeof val === "function" && val._reactive)
    {
      val = val();
    }
  ...
  {
    var setter = obj[key];
    if(typeof setter === "function" && setter._reactive)
    {
      setter(val);
    }
    else
...
var text = Object.freeze({
  ...
  var config = {
    ...
    unsafeDelimiters: unsafeDelimiters
  };
...
var transition = Object.freeze({
  ...
  function createAnchor(content, persist) {
    var anchor = document.createTextNode(persist ? ' ' : '');
  ...
  function defineReactive(obj, key, val) {
    ...
    var getter = function()
    {
      var value = func ? func.call(obj) : val;
      if (Dep.target) 
      {
        dep.depend();
        if (childOb) 
...
var path = Object.freeze({
  ...
  var pathTest1RE = /[^\w$\.][A-Za-z_$][\w$]*(\.[A-Za-z_$][\w$]*|\['.*?'\]|\[".*?"\]|\[\d+\]|\[[A-Za-z_$][\w\.$]*\])*/g;
  ...
  function rewrite(raw) {
    ...
    if (allowedKeywordsRE.test(path)) {
      ...
      return c + '__vm_getPath(scope,"' + path+'")';
    }
  }
  ...
  function compileGetter(exp) {
    ...
    body = (' ' + body).replace(pathTest1RE, rewrite).replace(restoreRE, restore);

    console.log("complexGetter "+body);
    return makeGetterFn(body);
  }

  function makePathGetterFn(path){
    console.log("simpleGetter "+path);
  ...
  function parseExpression(exp, needSet) {
    ...
    var res = { exp: exp };
    if(isSimplePath(exp)/* && exp.indexOf('[') < 0*/)
    {
      res.get = makePathGetterFn(exp);
    }
    else
    {
...
var template = Object.freeze({
  ...
  function compileTextNode(node, options) {
    ...
    var tokens = parseText(node.wholeText || node.data);
    if (!tokens) {
      return null;
    }
  ...
  function stateMixin (Vue) {
    ...
    Vue.prototype.$data = function(data)
    {
      if(arguments.length === 0)
      {
        return this._data;
    ...
    Vue.prototype._initComputed = function () {
      var gen = function(getter, setter)
      {
        return function(val)
        {
          if(arguments.length === 0)
  ...
  function installGlobalAPI (Vue) {
    ...
    Vue.setDelimiters = function(value)
    {
      delimiters = value;
      config.delimiters = value;

      compileRegex();
    }

    Vue.setUnsafeDelimiters = function(value)
    {
      unsafeDelimiters = value;
      config.unsafeDelimiters = value;
